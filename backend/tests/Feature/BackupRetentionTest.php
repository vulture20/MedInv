...
/**
 * BackupService::prune() (briefing 9.2). Manual backups (trigger='manual',
 * an admin explicitly clicking "Create backup now") are deliberately
 * exempt from both the count and max-age retention rules, and don't
 * consume a slot in the automatic-backup count either — reported because a
 * manual backup taken on purpose (e.g. right before a risky change) was
 * getting silently deleted by the automatic-backup retention policy, which
 * is meant to bound *unattended* backup accumulation, not backups an admin
 * asked for by name.
 */
class BackupRetentionTest extends TestCase
{
    ...
    {
        SystemSetting::set('backup.retention_count', 100);
    ...
    public function test_creating_a_manual_backup_survives_its_own_triggered_prune_under_a_tight_limit(): void
    {
        ...
        SystemSetting::set('backup.retention_max_age_days', 0);
