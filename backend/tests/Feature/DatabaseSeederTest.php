...
class DatabaseSeederTest extends TestCase
{
    ...
    /** GitHub issue #66: same reasoning as the Claude/OpenAI providers above — a third LLM-backed vendor, same cost/hallucination concern, so it must not be enabled just because the app was installed either. */
    public function test_seeding_creates_the_gemini_providers_disabled(): void
    {
        $this->seed();

        $table = (new MetadataPlugin)->getTable();
        $this->assertDatabaseHas($table, ['provider_key' => 'book.gemini', 'enabled' => false]);
        $this->assertDatabaseHas($table, ['provider_key' => 'cd.gemini', 'enabled' => false]);
    ...
    /** GitHub issue #130: same reasoning as the Amazon/Thalia providers above — JPC is a third Beta scraping vendor and must not be enabled just because the app was installed either. cd/dvd_bluray only — JPC doesn't sell books (see this issue's own scoping). */
